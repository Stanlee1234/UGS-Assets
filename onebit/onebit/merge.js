    ...
    if (filename) {
      ...
      return new Promise((resolve, reject) => {
        ...
        const check = setInterval(() => {
          ...
          if (buffer) {
            clearInterval(check);
            // Determine the correct Content-Type for the merged file so the
            // browser will accept and (if necessary) execute the resource.
            let contentType = 'application/octet-stream';
            if (filename.endsWith('.wasm') || filename.endsWith('.wasm.br')) contentType = 'application/wasm';
            else if (filename.endsWith('.js') || filename.endsWith('.js.br')) contentType = 'application/javascript';

            // Add Content-Encoding header for Brotli-compressed chunks so the
            // client knows it's compressed ('.br' files). Keep Content-Length
            // describing the raw byte length of the merged buffer.
            const headers = { 'Content-Type': contentType, 'Content-Length': buffer.byteLength.toString() };
            if (filename.endsWith('.br')) headers['Content-Encoding'] = 'br';

            resolve(new Response(buffer, { status: 200, statusText: 'OK', headers }));
    ...
    xhr.send = function(...args) {
      ...
      if (filename) {
        ...
        const waitForMerge = () => {
          const buffer = getMergedFile(filename);
          if (buffer) {
            // XHR consumers expect an ArrayBuffer for binary bodies. We also
            // set readyState/status so any XHR onload/onreadystatechange handlers
            // behave as if the XHR succeeded.
