...
ig.module('game.main').requires('impact.game', 'impact.font', 'game.menus', 'game.entities.enemy-missle', 'game.entities.enemy-mine', 'game.entities.enemy-destroyer', 'game.entities.enemy-oppressor', 'game.entities.player', 'plugins.impact-splash-loader').defines(function() {
  ...
  ZType = ig.Game.extend({
    ...
    hasShownNewHighscoreIndicator: false,
    highscoreNormal: 0,
    highscoreExpert: 0,
    ...
    difficulty: 'NORMAL',
    init: function() {
      this.highscoreNormal = parseInt(localStorage.getItem('highscore.normal')) || 0;
      this.highscoreExpert = parseInt(localStorage.getItem('highscore.expert')) || 0;
    ...
    reset: function() {
      ...
      this.hasShownNewHighscoreIndicator = false;
      this.score = 0;
      this.rs = 0;
      this.streak = 0;
      this.hits = 0;
    ...
    nextWave: function() {
      ...
      if (this.isNewHighscore) {
        this.hasShownNewHighscoreIndicator = true;
      }
    ...
    setGame: function() {
      this.reset();
      this.currentHighscore = (this.difficulty == 'NORMAL' ? this.highscoreNormal : this.highscoreExpert);
      this.menu = null;
    ...
    setGameOver: function() {
      if (this.difficulty == 'NORMAL' && this.score > this.highscoreNormal) {
        this.highscoreNormal = this.score;
        localStorage.setItem('highscore.normal', this.score);
      } else if (this.difficulty == 'EXPERT' && this.score > this.highscoreExpert) {
        this.highscoreExpert = this.score;
        localStorage.setItem('highscore.expert', this.score);
      }
    ...
    drawUI: function() {
      ...
      this.font.draw(s, ig.system.width - 4, ig.system.height - 30, ig.Font.ALIGN.RIGHT);
      if (this.score > this.currentHighscore) {
        this.isNewHighscore = true;
      }
      this.currentHighscore = Math.max(this.score, this.currentHighscore);
      var hs = this.currentHighscore.zeroFill(6);
      this.fontScore.draw('Highscore: ' + hs, ig.system.width - 5, ig.system.height - 8, ig.Font.ALIGN.RIGHT);
      if (this.waveEndTimer) {
        ...
        if (this.isNewHighscore && !this.hasShownNewHighscoreIndicator) {
          this.fontSelected.draw('NEW HIGHSCORE!', xs, ys + 56, ig.Font.ALIGN.CENTER);
        }
        ig.system.context.globalAlpha = 1;
      }
    },
    ...
    drawGameOver: function() {
      ...
      if (this.isNewHighscore) {
        this.font.draw('NEW HIGHSCORE!', xs, ys + 144, ig.Font.ALIGN.CENTER);
      }
      this.font.draw('Accuracy: ' + acc.round(1) + '%', xs, ys + 192, ig.Font.ALIGN.CENTER);
      this.font.draw('Words Per Minute: ' + wpm.round(1), xs, ys + 216, ig.Font.ALIGN.CENTER);
    }
  });
  ZType.MODE = {
    TITLE: 0,
